...
After pressing `Enter`, the process of determining aspects on the field using the neural network will start.
...
3. Launch from the project root (requires `Python 3.10` or above):
```shell
python -m src.main
```
...
3. Zaprun build command from the project root (will open an interface from which you can run the build):
```shell
