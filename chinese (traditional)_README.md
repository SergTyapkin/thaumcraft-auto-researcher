![GithubCI](https://github.com/SergTyapkin/thaumcraft-auto-researcher/actions/workflows/auto-translate-readme.yml/badge.svg)

[![](https://img.shields.io/badge/русский-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/README_TRANSLATIONS/ Russian_README.md)
[![](https://img.shields.io/badge/english-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/README_TRANSLATIONS/english_README.md)
[![](https://img.shields.io/badge/中文(简体)-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/RANumcraft&color=white)](https://github.com/SergTyapkin/RANumcraft&color=white)](https://github.com/SergTyapkin/RANumcraft-Uj-researcher/
[![](https://img.shields.io/badge/中文(传统)-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/RANumcraft&color=white)](https://github.com/SergTyapkin/RANumcraft&color=white)](https://github.com/SergTyapkin/RANumcraft-Uj-researcher/
[![](https://img.shields.io/badge/arabic(العربية)-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/thaumcraft-auto-researcher/b/UMEADMECD.
[![](https://img.shields.io/badge/español-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/README_TRANSLATIONS/spanish_READMEtoME.md)
[![](https://img.shields.io/badge/italiano-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/README_TRANSLATIONS/italian_README.md)
[![](https://img.shields.io/badge/Deutsch-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/README_TRANSLATIONS/dutch_README.md)
[![](https://img.shields.io/badge/hindi(हिन्दी)-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/thaumcraft-auto-researcher
[![](https://img.shields.io/badge/korean(한국어)-_?style=for-the-badge&logo=readme&color=white)](https://github.com/SergTyapkin/thaumcraft-auto-researcher


# Автоматический исследователь для Thaumcraft 4
> _**Thaumcraft**_ - мод для игры _Minecraft_, часто устанавливающийся в магические сор
## [下載 .exe 版本](https://github.com/SergTyapkin/thaumcraft-auto-researcher/releases)
[最新版本`v1.1.3`](https://github.com/SergTyapkin/thaumcraft-auto-researcher/releases/tag/v1.1.3)
<details>
<summary>更改日誌：</summary>

- 配置保存在AppData中。重啟時，不再需要重新配置應用程式
- 現在神經網路決定了桌面上的各個方面！
得益於此，研究速度提高了十倍以上。
- 由於其本地緩存，提高了神經網路的速度
- 新增了鍵盤快捷鍵以實現更精細的控制
- 新增不間斷研究模式

> `v1.1._` - 透過神經網路對錶上的方面進行配置，並且能夠由使用者更改
>
> `v1.0._` - 使用者在表上設定方面
>
> `v0._._` - 預發布 MVP 版本
</details>

---
該程式使用神經網絡，**自動解決**研究筆記並在研究表中排列**。
整個互動介面是半透明的，出現在所有視窗的上方。

該程式不會**以任何方式**與遊戲代碼交互，並且不會被反作弊程式檢測到。 
它所做的只是查看**螢幕上的像素**，並在神經網路的幫助下模擬**使用滑鼠和鍵盤的動作**，就像一個人在做一樣。

> [!重要]
> 如有任何問題、錯誤和建議，請寫信：[t.me/Tyapkin_S](https://t.me/tyapkin_s)

<details>
<summary>支援的插件清單（展開...）</summary>

- 魔法蜜蜂
- 禁忌魔法
- 阿瓦里蒂亞
- 格雷格科技
- 格雷科技新視野
- 奇術靴子
- 植物插件
- 極樂世界
- 奇術啟示
- 基本奇術
- 深淵工藝整合
</details>

https://github.com/user-attachments/assets/a2eaa3b7-c7fe-4fbc-9905-1b19a32d498f

---


# 如何使用這個？
### 預設 
> _程式第一次啟動後執行一次_
0. 從[releases](https://github.com/SergTyapk)下載程序在/thaumcraft-auto-researcher/releases）
1. 十字準線可移動的示範與驗證。 
只需將紅點移到黃點即可。
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/enroll.png?raw=true)
2. 您必須告訴程式研究表介面所在的位置。 
為此，需要移動黃色矩形的角，使其沿著表格的外周，如下面的螢幕截圖所示
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/find_table.png?raw=true)
3. 有必要讓程式更詳細地知道互動按鈕位於結界表內的位置。
為此，請移動所有點，如下面的螢幕截圖所示
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/setup_controls.png?raw=true)
4.選擇您的Thaumcraft版本和所有已安裝的插件
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/setup_version_and_addons.png?raw=true) 

完成所有這些步驟後，所有使用者選擇都保存在 `C://users/%USER%/.ThaumcraftAutoResearcher` 資料夾中，
下次啟動程式時無需執行此操作；將立即顯示下一步。
您可以隨時按下「Backspace」鍵返回配置

### 解決方面鏈
1. **將研究筆記**從左上角的庫存欄位放入研究桌欄位
按下“Enter”後，將開始使用神經網路確定現場情況的過程。
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/prepare_to_solving_aspects.png?raw=true)
將使用程式將發布的方面鏈自動生成解決方案
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/aspects_solved.png?raw=true)

> [!提示]
> 如果切面鏈太大或使用了您沒有的切面，請按「R」重新產生它
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/aspects_rerolled.png?raw=true)

> [!提示]
> 如果你需要在遊戲中做一些事情，讓遊戲不與程式介面重疊，可以按`Ctrl+Shift+Space`，然後
程式將暫停，直到您再次按下該組合鍵。
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/program_paused.png?raw=true)

> [!提示]
> 如果任何儲存格定義不正確，您可以按一下該儲存格並選擇它實際上應該是哪個儲存格一個。
之後，解決方案將自動重新生成
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/setup_table_aspects.png?raw=true)
2. **確保墨盒中有足夠的墨水**。如果它們用完，佈局方面的演算法將不會停止，
和研究筆記不會解決。
然後按下“Enter”，根據生成的鏈在表格上佈置方面的過程將開始。
3. **佈置完各個方面**後，研究筆記將被放入庫存中，
而不是它，而是將庫存中的下一個放在桌子上。
然後該過程將再次重複。這樣你就可以依序解決庫存中的大量紙幣
![image](https://github.com/SergTyapkin/thaumcraft-auto-researcher/blob/master/README_images/next_research_putted.png?raw=true)

> [!提示]
> - 為了繼續一項一項地檢查研究筆記，因為它們在清單中，您可以按“Ctrl+Enter”，然後
當學習下一個註解時，程式不會等待使用者使用「Enter」鍵確認，而是立即開始發布解決方案。

> [!提示]
> - 佈置切面時，提供組合鍵“Ctrl+Shift+Alt”，以防需要緊急終止程序。


----------


## 在未來的版本...
- 自動檢測表中可用的方面及其數量，並根據此資訊建立鏈。
- 根據遊戲中的 FPS 自適應速度
- 檢查佈置的鏈條的正確性
- 墨水罐狀態跟踪
- 在應用程式內翻譯成其他語言


----------

# 從原始碼運行：
1.安裝依賴項：
```shell
pip install -r requirements.txt
```

2.將項目src資料夾加入PYTHONPATH：
視窗：
```cmd
set "PYTHONPATH=$($CWD);$($PYTHONPATH)"
```
-Unix：
```cmd
export PYTHONPATH=$(cwd):$PYTHONPATH
```

3.從專案根運行（需要`Python 3.10`或更高版本）：
```shell
python -m src.main
```

---
## 將應用程式建置為 .exe 檔案
1. 安裝依賴項和建構器：
```shell
pip install -r requirements.txt
pip install auto-py-to-exe
```

2. ***\[可選步驟]*** 下載UPX（減少最終exe檔案的大小）
https://github.com/upx/upx/releases/


3. 電擊從專案根目錄執行建置命令（將開啟一個可以運行建置的介面）：
```shell
auto-py-to-exe -c .\pyinstaller_configs\autoPyToExe.json
```

4. ***\[可選步驟]*** 在 **高級** 部分中，指定 `--upx-dir` （包含 `upx.exe` 可執行檔的資料夾的位置）並執行構建。
編譯後的exe檔會出現在該目錄下的`output`資料夾中

---
### 特別感謝
- [Acak1221](https://github.com/acak1221) 用於創建程式使用的神經網絡