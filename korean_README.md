...
> `v1.2._` - 여러 신경망을 사용하여 모든 측면을 구성하고 누락된 부분을 제작
...
> _**Thaumcraft**_는 _Minecraft_ 게임의 모드로, 인기 있는 서버의 매직 모드 어셈블리에 자주 설치됩니다.

두 개의 신경망을 사용하여 화면의 측면을 결정하는 프로그램은 연구 테이블의 연구 노트를 **알고리즘적으로 해결하고 배열**합니다.
전체 상호 작용 인터페이스는 반투명하며 게임 창 상단에 나타납니다.

이 프로그램은 게임 코드와 **어떤 방식으로든** 상호 작용하지 않으며 치트 방지 프로그램에 의해 감지되지 않습니다.
이 기능이 하는 일은 **화면의 픽셀**을 보고, 신경망을 사용하여 화면의 어떤 측면이 있는지 확인하고, 마치 사람이 하는 것처럼 **마우스 및 키보드 동작을 모방**하는 것뿐입니다.
...
# 이것을 어떻게 사용하나요?
> [!주의]
> 사진과 설명이 최신 버전의 프로그램과 일치하지 않습니다. 유사한 시스템을 사용하지만 더 자동화되었습니다. 프로그램 내부의 지시 사항을 따르기만 하면 모든 것이 명확해집니다.
> 아래 지침을 반드시 업데이트할 예정이지만 지금은 그게 전부입니다.

### 사전 설정
> _프로그램 최초 시작 후 1회 실행_
0. [릴리스](https://github.com/SergTyapkin/thaumcraft-auto-researcher/releases)에서 프로그램을 다운로드하세요.
...
다음에 프로그램을 시작할 때 이 작업을 수행할 필요는 없습니다. 다음 단계가 즉시 표시됩니다.
...
> [!팁]
> 셀 중 하나라도 잘못 정의된 경우 해당 셀을 클릭하여 실제 셀이 무엇인지 선택할 수 있습니다.
...
3. 프로젝트 루트에서 빌드 명령을 실행합니다(빌드를 실행할 수 있는 인터페이스가 열립니다).
```shell
...
### 특별한 감사
- [Acak1221](https://github.com/acak1221) 솔루션의 측면을 결정하기 위한 신경망 생성
- [Limuranius](https://github.com/Limuranius) 테이블에서 측면과 그 수를 결정하기 위한 신경망을 만들고 신경망 실행을 위한 경량 시스템을 만드는 데 많은 작업을 하셨습니다.